// use fetch to retrieve the products and pass them init
// report any errors that occure in the fetch operation
...
function initialize(products) {
    ...
    // finalGroup will contain the products that need to be display after
    ...
    function selectCategory(e) {
        ...
        if(category.value === lastCategory && searchTerm.value.trim() === lastSearch) {
           return;
            ...
            // term, so we just set categoryGroup to the entire JSON object, then selecteProducts()
            if(category.value === 'All') {
                categoryGroup = products;
                selectProducts();
                // If a specific category is chosen, we need to filter out the products no in that
                // category, then put the remaining products inside categoryGroup, before running
                // selectProducts()
            } else {
                // the values in the <option> elements are uppercase, whereas the categories
                // store in the JSON(unde "type") are lowercase. We therefore need to convert
    ...
    function selectProducts() {
        ...
        // array - we don't want to filter the products - then updateDisplay().
        if(searchTerm.value.trim() === '') {
            finalGroup = categoryGroup;
            updateDisplay();
        } else {
            // Make sure the search term is converted to lower case before comparison. We've kept the
            // product name all lower case to keep things simple
            ...
            // (if the indexOf() result doesn't return-1, it means it is) - if it is, then push then push the product
    ...
    function updateDisplay() {
        ...
        if(finalGroup.length === 0) {
            ...
            // for each product we want to display, pass its products object to fetchBlob()
        } else {
    ...
    function fetchBlob(product) {
        // construct the URL path to the image from the product.image property
        ...
        fetch(url).then(function(response) {
            ...
            // that points to an object stored inside browser
    ...
    function showProduct(objectURL, product) {
        ...
        // Give <p> textContent equal to the product "price" property, with a $ sign in front
